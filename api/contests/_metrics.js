...
export async function refreshEntryMetrics(supabase, entry, { force = false } = {}) {
...
export async function refreshStaleEntryMetrics(supabase, entries, { limit = 20 } = {}) {
  let updated = 0;
  for (const entry of entries.slice(0, limit)) {
    const result = await refreshEntryMetrics(supabase, entry).catch(() => null);
