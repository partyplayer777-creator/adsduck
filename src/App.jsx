...
export default function App() {
  ...
  // 초기 history state 세팅 + URL ?c=ID 파라미터로 직접 공유 지원
  useEffect(() => {
  ...
  const handleNavigate = (target) => {
    if (page !== target) {
      // 홈으로 돌아갈 때 쿼리 파라미터 제거
      const cleanUrl = target === "home" ? window.location.pathname : window.location.href;
      window.history.pushState({ page: target }, "", cleanUrl);
    }
    setPage(target);
    setSelectedContest(null);
    if (target === "home") {
